...
class ArgumentHandler(Validator):
    ...
    def __init__(self, options, logfile=None, verbose=False, debug=False):
        ...
        self._auto_hull_shape = (
            options.auto_wedge, options.auto_tetra, options.auto_corner, options.auto_hepta)
        ...
        assert self._path_output is None or self.validate_dir(
            self._path_output, only_parent=True)
        ...
        if self._is_archived:  # .sment file
            ...
            if temp_directory is None:
                self._tmp_dir = tempfile.mkdtemp(
                    prefix="{}_".format(self._label))
            else:
                self._tmp_dir = tempfile.mkdtemp(
                    prefix="{}_".format(self._label), dir=temp_directory)
            ...
            self._directory_input = os.path.join(
                self._directory_input, blueprint_name)

            if self._path_output is not None:
                assert self._path_output.endswith(
                    ".sment"), "Expected '*.sment' file ending."
                assert not self.validate_file(
                    self._path_output, silent=True), "Output file exists. Overwriting files is not allowed, aborting."
                self._directory_output = os.path.join(
                    tempfile.mkdtemp(dir=self._tmp_dir), blueprint_name)
        else:
            self._tmp_dir = None
            ...
            assert self.validate_dir(
                self._directory_input, file_names=file_names), "Blueprint input path is invalid, aborting."
            if self._path_output is not None:
                self._directory_output = self._clean_dir_path(
                    self._path_output)
