...
class Validator(DefaultLogging):
    ...
    def validate_file(self, file_path, executable=False, key=None, silent=False):
        ...
        if parent_directory and not self.validate_dir(parent_directory, key=key, silent=silent):
            if not silent:
                self._logger.error(
                    "{}Directory of file does not exist: '{}'".format(prefix, parent_directory))
            return False
        ...
        if not os.path.isfile(file_path):
            if not silent:
                self._logger.error(
                    "{}File does not exist: '{}'".format(prefix, file_path))
            return False

        if executable and not os.access(file_path, os.X_OK):
            if not silent:
                self._logger.error(
                    "{}Permission error. File can not be executed '{}'".format(prefix, file_path))
            return False
        return True
    ...
    def validate_characters(self, text, legal_alphabet=string.printable, key=None, silent=False):
        ...
        if not set_legal_alphabet.issuperset(set_text):
            if not silent:
                ...
                self._logger.error(
                    "{}Invalid characters: '{}'".format(prefix, ", ".join(difference)))
            return False
        return True
    ...
    def validate_dir(self, directory, only_parent=False, sub_directories=None, file_names=None, key=None, silent=False):
        ...
        if directory == '':
            if not silent:
                self._logger.error(
                    "{}Invalid directory: '{}'".format(prefix, directory))
            return False
        ...
        if not os.path.isdir(parent_directory):
            if not silent:
                self._logger.error(
                    "{}Directory does not exist: '{}'".format(prefix, parent_directory))
            return False

        if not only_parent and not os.path.isdir(directory):
            if not silent:
                self._logger.error(
                    "{}Directory does not exist: '{}'".format(prefix, directory))
            return False
    ...
    def validate_number(self, digit, minimum=None, maximum=None, zero=True, key=None, silent=False):
        ...
        if minimum and digit < minimum:
            if not silent:
                self._logger.error("{}Invalid digit, must be bigger than {}, but was {}".format(
                    prefix, minimum, digit))
            return False

        if maximum and digit > maximum:
            if not silent:
                self._logger.error("{}Invalid digit, must be smaller than {}, but was {}".format(
                    prefix, maximum, digit))
            return False

        if not zero and digit == 0:
            if not silent:
                self._logger.error(
                    "{}Invalid digit, must not be {}".format(prefix, digit))
            return False
        return True
    ...
    def validate_free_space(
        ...
        # print required_space, required_space_in_bytes, required_space_in_kb,
        # required_space_in_mb, required_space_in_gb
    ...
    def get_available_file_path(self, proposed_path):
        ...
        assert self.validate_dir(
            proposed_path, only_parent=True), "Bad path '{}'".format(proposed_path)
        ...
        while os.path.exists(new_path):
            new_path = "{base}_{index}{ext}".format(
                base=path, index=index, ext=extension)
            index += 1
        return new_path
