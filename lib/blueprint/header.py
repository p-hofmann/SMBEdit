...
class Statistics(object):
    ...
    def read_statistics(self, input_stream):
        ...
        assert self.version in self._valid_versions, "Unsupported Statistic v{}".format(
            self.version)
...
class Header(DefaultLogging, BlueprintUtils):
    ...
    def set_class(self, entity_class):
        ...
        assert self.type in [
            0, 2], "Can not set class. Blueprint is not a ship/station."
        assert entity_class in BlueprintUtils._entity_classification[
            self.type], "Unknown class id: {}.".format(entity_class)
    ...
    def remove(self, block_id, quantity=None):
        ...
        assert quantity is None or self.block_id_to_quantity[
            block_id] >= quantity, (self.block_id_to_quantity[block_id], quantity)
        if quantity is None:
