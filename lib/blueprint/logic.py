...
class Logic(DefaultLogging, BlueprintUtils):
    ...
    def _read_dict_of_groups(self, input_stream):
        ...
        for entry_index in range(0, number_of_groups):
            ...
            block_id_to_positions[
                block_id] = self._read_set_of_positions(input_stream)
    ...
    def _read_list_of_controllers(self, input_stream):
        ...
        for entry_index in range(0, number_of_controllers):
            ...
            controller_position_to_groups[
                position] = self._read_dict_of_groups(input_stream)
    ...
    def _read_file(self, input_stream):
        ...
        self._controller_position_to_block_id_to_block_positions = self._read_list_of_controllers(
            input_stream)
    ...
    def tilt_turn(self, index_turn_tilt):
        ...
        new_data = {}
        for controller_position, groups in self._controller_position_to_block_id_to_block_positions.iteritems():
            new_controller_position = self._tilt_turn_position(
                controller_position, index_turn_tilt)
            new_data[new_controller_position] = {}
            for block_id, positions in groups.iteritems():
                new_data[new_controller_position][block_id] = set()
                for position in positions:
                    new_position = self._tilt_turn_position(
                        position, index_turn_tilt)
                    new_data[new_controller_position][
                        block_id].add(new_position)
    ...
    def set_link(self, controller_position, group_id, positions):
        ...
        if controller_position not in self._controller_position_to_block_id_to_block_positions:
            self._controller_position_to_block_id_to_block_positions[
                controller_position] = {}
        self._controller_position_to_block_id_to_block_positions[
            controller_position][group_id] = positions
    ...
    def move_center(self, direction_vector, entity_type=0):
        ...
        new_dict = {}
        for controller_position, groups in self._controller_position_to_block_id_to_block_positions.iteritems():
            new_controller_position = self.vector_subtraction(
                controller_position, direction_vector)
            # replaced block
            if entity_type == 0 and new_controller_position == (16, 16, 16):
                continue
            # core
            if entity_type == 0 and controller_position == (16, 16, 16):
            ...
            for block_id, positions in groups.iteritems():
                ...
                for block_position in positions:
                    new_block_position = self.vector_subtraction(
                        block_position, direction_vector)
                    # replaced block
                    if entity_type == 0 and new_block_position == (16, 16, 16):
                        continue
                    new_dict[new_controller_position][
                        block_id].add(new_block_position)
    ...
    def _update_groups(self, controller_position, block_id, smd):
        ...
        positions = list(self._controller_position_to_block_id_to_block_positions[
                         controller_position][block_id])
        for position in positions:
            if not smd.has_block_at_position(position):
                self._controller_position_to_block_id_to_block_positions[
                    controller_position][block_id].remove(position)
        if len(self._controller_position_to_block_id_to_block_positions[controller_position][block_id]) == 0:
            self._controller_position_to_block_id_to_block_positions[
                controller_position].pop(block_id)
    ...
    def update(self, smd):
        ...
        for controller_position in self._controller_position_to_block_id_to_block_positions.keys():
            groups = self._controller_position_to_block_id_to_block_positions[
                controller_position]
            ...
            if len(self._controller_position_to_block_id_to_block_positions[controller_position]) == 0:
                self._controller_position_to_block_id_to_block_positions.pop(
                    controller_position)
    ...
    def update_link(self, old_position, new_position):
        ...
        if self._debug:
            self._logger.debug(
                "update_link: {} -> {}".format(old_position, new_position))
        if old_position in self._controller_position_to_block_id_to_block_positions:
            groups = self._controller_position_to_block_id_to_block_positions.pop(
                old_position)
            self._controller_position_to_block_id_to_block_positions[
                new_position] = groups
        for controller_position, groups in self._controller_position_to_block_id_to_block_positions.iteritems():
            for block_id, positions in groups.iteritems():
                if old_position in positions:
                    self._controller_position_to_block_id_to_block_positions[
                        controller_position][block_id].remove(old_position)
                    self._controller_position_to_block_id_to_block_positions[
                        controller_position][block_id].add(new_position)
    ...
    def to_stream(self, output_stream=sys.stdout):
        ...
        output_stream.write("Controllers: {}\n".format(
            len(self._controller_position_to_block_id_to_block_positions)))
        output_stream.write("\n")
