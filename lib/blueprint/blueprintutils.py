...
class BlueprintUtils(object):
    ...
    def get_entity_classification_name(entity_type, entity_classification):
        ...
        assert entity_classification in BlueprintUtils._entity_classification[
            entity_type]
    ...
    def _get_hull_details(block_id):
        ...
        shape = shape.strip()
        assert shape in BlueprintUtils._block_shapes, "Unknown Shape: '{}'".format(
            shape)
    ...
    def _get_hulls_dict():
        ...
        for block_id in BlueprintUtils._block_ids["hull"].keys():
            ...
            if "Glass" in hull_name:
                continue
            hull_type, color, shape_id = BlueprintUtils._get_hull_details(
                block_id)
    ...
    def _is_block_style_3(block_id):
        ...
        """
        style_3_categories = [
            "ores", "shards", "crafting", "minerals", "rails"]
        for category in style_3_categories:
            if block_id in BlueprintUtils._block_ids[category]:
                return True
        # and not Mushroom
        if block_id in BlueprintUtils._block_ids["plants"] and block_id != 104:
            return True
    ...
    def _is_block_style_0(block_id):
        ...
        """
        style_0_categories = [
            "hull", "circuits", "motherboards", "ingots", "crystals", "decorative"]
        ...
        for category in style_0_categories:
            if block_id in BlueprintUtils._block_ids[category]:
                ...
                block_name = BlueprintUtils.get_block_name_by_id(
                    block_id).lower()
                for shape in shapes:
                    if shape in block_name:
                        return False
    ...
    def get_block_style(block_id):
        ...
        @rtype: int
        """
        assert BlueprintUtils.is_known_id(
            block_id), "Unknown block id: {}".format(block_id)
        block_name = BlueprintUtils.get_block_name_by_id(block_id).lower()
    ...
    def _tilt_turn_position(position, tilt_index):
        ...
        new_block_position = BlueprintUtils.vector_subtraction(
            position, BlueprintUtils._core_position)
        new_block_position = (
        ...
        new_block_position = BlueprintUtils.vector_addition(
            new_block_position, BlueprintUtils._core_position)
