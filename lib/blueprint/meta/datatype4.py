...
class DataType4(DefaultLogging):
    ...
    def _read_unknown_d4_stuff(self, input_stream, unknown_number):
        ...
        self._logger.debug(
            "unknown_d4_stuff string: '{}'".format(unknown_string))
        # if unknown_number != 0:
    ...
    def read(self, input_stream, version):
        ...
        if version >= (0, 0, 0, 2):
            ...
            for some_index in range(list_size_of_unknown_stuff):
                self._entity_unknown_list_of_tuple[
                    some_index] = self._read_unknown_d4_stuff(input_stream, unknown_number)
        ...
        for turret_index in range(amount_of_docked_entities):
            ...
            if tag_size > 0:
                self._docked_entity[int(dock_index)] = TagManager(
    ...
    def write(self, output_stream, version, relative_path, compressed=False):
        ...
        if version >= (0, 0, 0, 2):
            ...
            list_size_of_unknown_stuff = len(
                self._entity_unknown_list_of_tuple)
            ...
            for index in sorted(self._entity_unknown_list_of_tuple.keys()):
                self._write_unknown_d4_stuff(
                    output_stream, self._entity_unknown_list_of_tuple[index], unknown_number)

        output_stream.write_int32_unassigned(len(self._docked_entity))
        for dock_index in sorted(self._docked_entity.keys()):
            new_relative_directory = os.path.join(
                relative_path, "ATTACHED_{}".format(dock_index))
    ...
    def _get_docker_related_loaction_tags(self):
        ...
        for docked_entity_index in self._docked_entity.keys():
            ...
            assert tag_docked_entity_location.id == - \
                10, tag_docked_entity_location.id
