...
class Blueprint(DefaultLogging, BlueprintUtils):
    ...
    def write(self, directory_blueprint, relative_path=None):
        ...
        assert os.path.exists(
            directory_blueprint), "Output directory failed to be created."
    ...
    def move_center_by_vector(self, direction_vector):
        ...
        min_vector, max_vector = self.smd3.move_center(
            direction_vector, self.header.type)
    ...
    def link_salvage_modules(self):
        ...
        if salvage_computer_count == 1:
            self.logic.set_link(
                position_salvage_computers.pop(), 24, position_salvage_modules)
            return
        ...
        for group_index in groups.keys():
            self.logic.set_link(
                position_salvage_computers[group_index], 24, groups[group_index])
