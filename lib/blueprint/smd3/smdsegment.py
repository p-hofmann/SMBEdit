...
class SmdSegment(DefaultLogging, BlueprintUtils):
    ...
    def __init__(self, blocks_in_a_line=32, logfile=None, verbose=False, debug=False):
        ...
        self._blocks_in_an_area = self._blocks_in_a_line * \
            self._blocks_in_a_line
        self._blocks_in_a_cube = self._blocks_in_an_area * \
            self._blocks_in_a_line
        self.version = 2
        self.timestamp = 0
        self.position = None
    ...
    def _read_block_data(self, input_stream):
        ...
        decompressed_data = zlib.decompress(
            input_stream.read(self.compressed_size))
        self.block_index_to_block = {}
        for block_index in range(0, len(decompressed_data)/3):
            position = block_index * 3
            block = SmdBlock(debug=self._debug)
            int_24bit = ByteStream.unpack_int24(
                decompressed_data[position:position+3])
    ...
    def _write_block_data(self, output_stream):
        ...
        if not self.has_valid_data:
            self.compressed_size = 0
            output_stream.write_int32_unassigned(
                self.compressed_size)   # 4 byte
        else:
            byte_string = ""
            ...
            for block_index in range(0, self._blocks_in_a_cube):
                if block_index in set_of_valid_block_index:
                    block_int_24 = self.block_index_to_block[
                        block_index].get_int_24bit()
        ...
        # this should fill the skipped positions with \0
    ...
    def get_block_index_by_block_position(self, position):
        ...
        return \
            ...
            (position[2] % self._blocks_in_a_line) * \
            self._blocks_in_an_area
        ...
        for block_index in self.block_index_to_block.keys():
            if self.block_index_to_block[block_index].get_id() == block_id:
                if compatible:
                    ...
                    self.block_index_to_block[
                        block_index].set_hit_points(replace_hp)
                else:
        ...
        for block_index in self.block_index_to_block.keys():
            ...
            if block_id not in self._replace_cache_positive:
                block_hull_type, color, shape_id = self._get_hull_details(
                    block_id)
                # not replaced
                if hull_type is not None and hull_type != block_hull_type:
                    self._replace_cache_negative.add(block_id)
                    continue
                new_block_id = self.get_hull_id_by_details(
                    new_hull_type, color, shape_id)
                self._replace_cache_positive[block_id] = new_block_id
            self.block_index_to_block[block_index].set_id(
                self._replace_cache_positive[block_id])
            self.block_index_to_block[block_index].set_hit_points(
                self.get_hp_by_hull_type(new_hull_type))
    ...
    def update(self, entity_type=0):
        ...
        for block_index in list_of_block_index:
            ...
            if not self.is_valid_block_id(block.get_id(), entity_type):
                self.remove_block(
                    self.get_block_position_by_block_index(block_index))
                continue
    ...
    def remove_block(self, block_position):
        ...
        # print "deleting",
        # self.get_block_name_by_id(self.block_index_to_block[block_index].get_id())
        self.block_index_to_block.pop(block_index)
        if self.get_number_of_blocks() == 0:
            self._logger.debug(
                "Segment {} has no more blocks.".format(self.position))
            self.has_valid_data = False
    ...
    def to_stream(self, output_stream=sys.stdout):
        ...
        output_stream.write("Segment: {} '{}' ({})\n".format(
            ...
            datetime.datetime.fromtimestamp(
                self.timestamp/1000.0).strftime('%Y-%m-%d %H:%M:%S'),
            self.version,
        ))
        output_stream.flush()
        if self._debug:
            for block_index in sorted(self.block_index_to_block.keys()):
                output_stream.write(
                    "{}\t".format(self.get_block_position_by_block_index(block_index)))
