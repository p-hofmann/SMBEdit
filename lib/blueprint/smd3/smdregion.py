...
from lib.blueprint.smd3.smdsegment import SmdSegment
...
class SmdRegion(DefaultLogging, BlueprintUtils):
    ...
    def __init__(self, segments_in_a_line=16, blocks_in_a_line=32, logfile=None, verbose=False, debug=False):
        ...
        self._segments_in_an_area = self._segments_in_a_line * \
            self._segments_in_a_line  # 256
        self._segments_in_a_cube = self._segments_in_an_area * \
            self._segments_in_a_line  # 4096
    ...
    def _read_region_header(self, input_stream):
        ...
        assert self.version == (
            2, 0, 0, 0), "Unsupported smd version: {}".format(self.version)
    ...
    def _write_region_header(self, output_stream):
        ...
        for position, segment in self.position_to_segment.iteritems():
            ...
            segment_index_to_size[segment_index] = self.position_to_segment[
                position].compressed_size + segment_header_size
        ...
        for segment_index in range(0, self._segments_in_a_cube):
            ...
            self._write_segment_index(
                seg_id, segment_index_to_size[segment_index], output_stream)
    ...
    def _write_file(self, output_stream):
        ...
        """
        output_stream.seek(
            4+self._segments_in_a_cube*4)  # skip header: version(4byte) + 4096 segment index (4 byte)
    ...
    def get_segment_index_by_position(self, segment_position):
        ...
        # max_blocks 32768
        offset = self._blocks_in_a_line_in_a_segment * \
            self._segments_in_a_line / 2
        tmp = [0, 0, 0]
    ...
    def replace_hull(self, new_hull_type, hull_type=None):
        ...
        for segment_position in self.position_to_segment.keys():
            self.position_to_segment[segment_position].replace_hull(
    ...
    def replace_blocks(self, block_id, replace_id, replace_hp, compatible=False):
        ...
        """
        for segment_position in self.position_to_segment.keys():
            self.position_to_segment[segment_position].replace_blocks(
    ...
    def _remove_empty_segments(self):
        ...
        for position_segment in list_of_positions:
            if self.position_to_segment[position_segment].get_number_of_blocks() == 0:
                self._logger.debug(
                    "'remove' Removing empty segment {}.".format(position_segment))
    ...
    def remove_block(self, block_position):
        ...
        position_segment = self.get_segment_position_of_position(
            block_position)
    ...
    def add(self, block_position, block):
        ...
        if position_segment not in self.position_to_segment:
            self.position_to_segment[position_segment] = SmdSegment(
                ...
                debug=self._debug)
            self.position_to_segment[
                position_segment].set_position(position_segment)
