...
class SMBEdit(ArgumentHandler):
    ...
    def _move_center_or_core(self, blueprint):
        ...
        if ',' in self._move_center:  # vector
            ...
            assert self._move_center.isdigit(), "Bad block id: '{}'".format(
                self._move_center)
    ...
    def _replace_blocks(self, blueprint):
        ...
        assert old_block_id.isdigit(), "Bad old block id: '{}'".format(
            old_block_id)
        old_block_id = int(old_block_id)

        assert ':' in suffix, "Bad replace: '{}'".format(self._replace)
        replace_id, replace_hp = suffix.split(':')
        assert replace_id.isdigit(), "Bad replace block id: '{}'".format(
            replace_id)
        assert replace_hp.isdigit(), "Bad replace block hp: '{}'".format(
            replace_hp)
    ...
    def _replace_hull_and_armor(self, blueprint):
        ...
        @type blueprint: Blueprint
        """
        assert isinstance(self._replace_hull, str), "Bad replace hull: '{}'".format(
            self._replace_hull)
        assert ':' in self._replace_hull, "Bad hull replace: '{}'".format(
            self._replace_hull)
        old_hull_type, new_hull_type = self._replace_hull.split(':', 1)
        assert new_hull_type in self._char_to_hull_type, "Bad replace hull type: '{}'".format(
            new_hull_type)
        new_hull_type = self._char_to_hull_type[new_hull_type]
        if old_hull_type != "":
            assert old_hull_type in self._char_to_hull_type, "Bad hull type: '{}'".format(
                old_hull_type)
    ...
    def run_commands(self, directory_input=None, directory_output=None, blueprint_path=None):
        ...
        assert self.validate_dir(
            directory_input, file_names=file_names), "Blueprint directory is invalid, aborting."
        if directory_output is not None:
            assert self.validate_dir(
                directory_output, only_parent=True, key="-o"), "Output Blueprint directory is invalid, aborting."
            if os.path.exists(directory_output):
                if len(os.listdir(directory_output)) > 0:
                    raise Exception(
                        "Blueprint found in output directory, aborting to prevent overwriting.")
            else:
        ...
        if self._docked_entities or not is_docked_entity:
            ...
            if True in self._auto_hull_shape:
                self._logger.info(
                    "Automatically set a shape to hull blocks...")
        ...
        if directory_output is not None:
            ...
            relative_path = os.path.relpath(
                directory_output, os.path.dirname(blueprint_path))
            blueprint.write(directory_output, relative_path=relative_path)


def main():
    options = ArgumentHandler.get_parser_options(
        label=SMBEdit.get_label(), version=__version__)
